...
The criteria builder component should implement a chip-based visual interface as shown in the reference design:

![Criteria Builder Panel Component](./criteria-builder-panel-component.png)

### Visual Design Principles

- **chip-based Interface**: All criteria elements (fields, operators, values, functions, groups) are represented as interactive chips
- **Hierarchical Layout**: Groups contain nested chips with clear visual hierarchy
- **Interactive Elements**: Each chip includes hover states, delete icons, and editing capabilities
...
### chip Types Implementation

Based on the reference design, the component should render:

1. **Field chips**: Display field names (e.g., "Close", "Volume") with appropriate styling
2. **Operator chips**: Show comparison operators (>, <, =, etc.) and logical operators (AND, OR, NOT)
3. **Value chips**: Display literal values with type indicators
4. **Function chips**: Show function calls with parameter chips nested inside
5. **Group chips**: Contain multiple chips with logical operator indicators and brace toggles
6. **Action chips**: Include delete icons and editing controls
...
1. **Given** a user has created basic conditions, **When** they add a group chip, **Then** they can select AND/OR/NOT operators and drag conditions into the group
...
1. **Given** a user wants to add a function, **When** they select a function chip, **Then** they can configure parameters through inline editing or popup editor
...
### User Story 4 - Interactive chip Management (Priority: P2)

A user needs to modify, reorder, and delete criteria elements through intuitive chip interactions including drag-and-drop, inline editing, and visual feedback.
...
**Independent Test**: Can be fully tested by creating criteria and then modifying them through chip interactions, verifying all changes are reflected in DSL output.

**Acceptance Scenarios**:

1. **Given** a user has created criteria chips, **When** they hover over chips, **Then** delete icons appear and can be clicked to remove elements
2. **Given** a user has multiple criteria elements, **When** they drag and drop chips, **Then** the order updates and DSL output reflects the new arrangement
3. **Given** a user double-clicks a group chip, **When** they perform the action, **Then** the group collapses/expands with visual indication
...
- **FR-002**: System MUST render criteria as interactive chips matching the reference design, representing fields, operators, values, functions, and groups with proper visual hierarchy
- **FR-003**: Users MUST be able to create, edit, and delete individual criteria elements through chip interactions
...
- **FR-016**: System MUST implement visual design consistent with the reference image, including chip styling, spacing, colors, and interactive states
...
- **SC-011**: Visual design matches reference image with 95% accuracy in chip styling, layout, and interactive states
...
- chip-based UI paradigm is intuitive for target user base (financial analysts)
